...
%
% The assumed double-layer capacitances of the electrodes.
%
%   contactSurfaces (:,1) zeffiro.SuperNode = zeffiro.SuperNode.empty
%
% The contact surfaces of the electrodes.
%
%   doubleLayerResistances (:,1) double { mustBeNonnegative, mustBeReal, mustBeFinite } = []
%
% The real-valued resistances of the capacitive electrode double layer.
%
%   frequencies (:,1) double { mustBeNonnegative } = []
%
% Stimulation frequencies in the case of tACS.
%
%   contactResistances (:,1) double { mustBeNonnegative, mustBeReal, mustBeFinite } = []
%
% The real-valued contact resistances of the electrodes.
%

    properties
        capacitances (:,1) double { mustBeNonnegative } = [] % Capacitances of the electrodes
        contactSurfaces (:,1) zeffiro.SuperNode = zeffiro.SuperNode.empty
        doubleLayerResistances (:,1) double { mustBeNonnegative, mustBeReal, mustBeFinite } = []
        frequencies (:,1) double { mustBeNonnegative } = []
        contactResistances (:,1) double { mustBeNonnegative, mustBeReal, mustBeFinite } = []
    ...
    methods
        ...
        %
            ...
            arguments
                kwargs.capacitances= []
                kwargs.contactSurfaces = zeffiro.SuperNode.empty
        ...
        %
            ...
            Z = zeffiro.impedanceParallel (self.doubleLayerResistances, self.frequencies, 0, self.capacitances) + self.contactResistances ;
        ...
        %
            ...
            if all ( isinf ( self.effectiveImpedances ) )
                ...
                M = zeffiro.ElectrodeModel.PEM ;

            else

                M = zeffiro.ElectrodeModel.CEM ;
