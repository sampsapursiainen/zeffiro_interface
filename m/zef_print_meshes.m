...
if isequal(zef.volumetric_distribution_mode,1)
    volumetric_distribution = zef.reconstruction;
elseif isequal(zef.volumetric_distribution_mode,2)
    variable_index_aux = zef.mesh_visualization_parameter_selected;
...
elseif isequal(zef.volumetric_distribution_mode,3)
    variable_index_aux = zef.mesh_visualization_parameter_selected;
    [~, variable_name_aux]  = zef_get_profile_parameters(zef,variable_index_aux);
    volumetric_distribution = repmat(eval([variable_name_aux '(:,1)'])',3,1)/sqrt(3);
...
file_index = zef.file_index;
file_name = zef.file;
file_path = zef.file_path;
snapshot_resolution = round([zef.snapshot_vertical_resolution zef.snapshot_horizontal_resolution]);

c_va = camva(zef.h_axes1);
c_pos = campos(zef.h_axes1);
c_ta = camtarget(zef.h_axes1);
c_p = camproj(zef.h_axes1);
c_u = camup(zef.h_axes1);

if ismember(zef.on_screen, [0,1]) && not(zef.visualization_type==3)
    ...
    if iscell(volumetric_distribution) &  zef.visualization_type == 2 & is_video & file_index == 4
        avi_file_temp = [file_path file_name(1:end-4) '_temp.avi'];
        avi_file = [file_path file_name];
        video_quality = str2num(num2str(zef.video_codec));
    ...
    aux_scale_val = zef.sensors_visual_size;
    ...
    if size(sensors,2) == 6 & ismember(zef.imaging_method, [1 4 5])
        electrode_model = 2;
    else
        electrode_model = 1;
    end
    ...
    if eval(['zef.' sensor_tag '_visible'])
        ...
        if zef.cp_on || zef.cp2_on || zef.cp3_on
            if zef.cp_mode == 1
        ...
        %April 2021
        if not(zef.attach_electrodes)
            sensors_name_points = sensors(:,1:3);
        end
        sensors_aux = sensors;
        %April 2021

        if electrode_model == 1 & zef.attach_electrodes & ismember(zef.imaging_method,[1 4 5])
            sensors = zef_attach_sensors_volume(zef,sensors,'mesh',sensors_get_functions);
        elseif electrode_model==2 & zef.attach_electrodes & ismember(zef.imaging_method,[1 4 5])
            sensors = zef_attach_sensors_volume(zef,sensors,'mesh',sensors_get_functions);
        ...
        if electrode_model == 1 | not(ismember(zef.imaging_method,[1,4,5]))
            for i = 1 : size(sensors,1)
                h = surf(sensors(i,1) + X_s, sensors(i,2) + Y_s, sensors(i,3) + Z_s);
                h.Tag = 'sensor';
                h.EdgeColor = 'none';
                %April 2021
                if eval(['zef.' zef.current_sensors '_names_visible'])
                ...
                set(h,'facealpha',zef.layer_transparency);
            end
        else
            %April 2021
            if eval(['zef.' zef.current_sensors '_names_visible'])
            ...
            if not(isempty(sensors))
                ...
                % set(h,'ambientstrength',0.7);
                set(h,'facealpha',zef.layer_transparency);
                set(h,'edgealpha',zef.layer_transparency);
            end
    ...
    if ismember(zef.visualization_type, [2,4])
        if ismember(zef.volumetric_distribution_mode, [1,3])
            if not(isempty(zef.source_interpolation_ind))
                s_i_ind = eval('zef.source_interpolation_ind{1}');
            end
        elseif ismember(zef.volumetric_distribution_mode, [2,4])
    ...
    if iscell(volumetric_distribution) &&  zef.visualization_type == 2
        length_reconstruction_cell = length(volumetric_distribution);
        frame_start = zef.frame_start;
        frame_stop = zef.frame_stop;
        frame_step = zef.frame_step;
        ...
        for f_ind = frame_start : frame_step : frame_stop
            ...
            if ismember(zef.reconstruction_type, 6)
        ...
        if not(ismember(zef.reconstruction_type, [6]))
            if zef.inv_scale == 1
                min_rec_log10 = 20*log10(max(min_rec,max_abs_reconstruction/zef.inv_dynamic_range));
                max_rec = -min_rec_log10 + 20*log10(max(max_rec,max_abs_reconstruction/zef.inv_dynamic_range));
                min_rec = 0;
            elseif zef.inv_scale == 2
                min_rec = (max(min_rec,max_abs_reconstruction/zef.inv_dynamic_range));
                max_rec = (max(max_rec,max_abs_reconstruction/zef.inv_dynamic_range));
            elseif zef.inv_scale == 3
                min_rec = sqrt(max(min_rec,max_abs_reconstruction/zef.inv_dynamic_range));
                max_rec = sqrt(max(max_rec,max_abs_reconstruction/zef.inv_dynamic_range));
            end
        end
    elseif  zef.visualization_type == 2
        %s_i_ind = eval('zef.source_interpolation_ind{1}');
    ...
    if ismember(zef.visualization_type,[2])
        if  iscell(volumetric_distribution) & zef.visualization_type == 2
    ...
    for f_ind = frame_start : frame_start
        ...
        %**************************************************************************
        if ismember(zef.visualization_type,[2,4])
            if ismember(zef.volumetric_distribution_mode, [2,4])
                active_compartment_ind_aux = [1:eval('size(zef.tetra,1)')]';
                active_compartment_ind = [1:length(I_aux)]';
                I_2  = I_aux;
            else
                active_compartment_ind_aux = zef.active_compartment_ind;
            ...
            if zef.use_parcellation
                ...
                p_rec_aux = ones(size(nodes,1),1)*zef.layer_transparency;
                p_cell = cell(0);
                for p_ind = selected_list
                    ...
                    p_rec_aux(unique(surface_triangles(I_3(p_ind_aux),:))) = zef.brain_transparency;
                end
            end
        end

        if ismember(zef.visualization_type,[4])
            reconstruction = reconstruction_p_1;
            min_rec = 1;
            max_rec = size(zef.parcellation_colormap,1);
        end
        ...
        if ismember(zef.visualization_type,[2])
            ...
            if ismember(zef.reconstruction_type, [2 3 4 5 6 7])
        ...
        if ismember(zef.visualization_type,[2,4])
            ...
            if not(ismember(zef.visualization_type,[4]))
                if zef.use_parcellation
            ...
            set(h_fig_aux,'colormap', eval([colormap_cell{zef.inv_colormap} '(' num2str(colortune_param) ',' num2str(colormap_size) ')']));
            ...
            if zef.brain_transparency < 1 || zef.use_parcellation
        ...
        for i = 1 : n_compartments
            ...
            if visible_vec(i)
                ...
                if not(isempty(I_3))
                    ...
                    if not(ismember(zef.visualization_type,[2,4])) || not(ismember(i,aux_active_compartment_ind))
                        set(h_surf,'facealpha',zef.layer_transparency);
                    end
                    lighting phong;
                end
            end
        ...
        if iscell(volumetric_distribution) &  zef.visualization_type == 2
            view(zef.azimuth,zef.elevation);
        else
            view(get(zef.h_axes1,'view'));
        end
        ...
        if zef.visualization_type == 2
            ...
            h_text = text(0, 0.5, ['Time: ' num2str(zef.inv_time_1 + zef.inv_time_2/2 + frame_step*(f_ind - 1)*zef.inv_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
    ...
    for f_ind = frame_start + frame_step : frame_step : frame_stop
        ...
        zef_set_sliders_print(1,h_axes_image);
        camorbit(frame_step*zef.orbit_1/movie_fps,frame_step*zef.orbit_2/movie_fps);
    ...
    if ismember(zef.visualization_type,[3,4])
        if iscell(volumetric_distribution)
            length_reconstruction_cell = length(volumetric_distribution);
        else
            length_reconstruction_cell = 1;
    ...
    if ismember(zef.visualization_type,[5])
        if iscell(zef.top_reconstruction)
            length_reconstruction_cell = length(zef.top_reconstruction);
        else
            length_reconstruction_cell = 1;
        end
        frame_start = zef.frame_start;
    ...
    if iscell(volumetric_distribution) &  zef.visualization_type == 3 & is_video & file_index == 4
        avi_file_temp = [file_path file_name(1:end-4) '_temp.avi'];
        avi_file = [file_path file_name];
        video_quality = str2num(num2str(zef.video_codec));
        h_aviobj = VideoWriter(avi_file_temp);
    ...
    if iscell(zef.top_reconstruction) &  zef.visualization_type == 5 & is_video & file_index == 4
        avi_file_temp = [file_path file_name(1:end-4) '_temp.avi'];
        avi_file = [file_path file_name];
        video_quality = str2num(nu2str(zef.video_codec));
        h_aviobj = VideoWriter(avi_file_temp);
        h_aviobj.Quality = video_quality;
        h_aviobj.FrameRate = movie_fps;
        open(h_aviobj);
    ...
    if ismember(zef.visualization_type, [3,4])
        ...
        elseif ismember(zef.volumetric_distribution_mode, [2,4])
            s_i_ind_2 = zef.active_compartment_ind;
        end
    end
    ...
    if ismember(zef.visualization_type,[3])
    ...
    if ismember(zef.visualization_type,[5])
        ...
        if iscell(zef.top_reconstruction)
            ...
            frame_step = 1;
            number_of_frames = 1;
            reconstruction = zef.top_reconstruction;
            reconstruction = reconstruction(:);
            max_abs_reconstruction = max([max_abs_reconstruction ; max(reconstruction,[],"all")]);
            min_rec = min([min_rec ; min(reconstruction,[],"all")]);
            max_rec = max_abs_reconstruction;
    ...
    if ismember(zef.imaging_method, [1 4 5])  & size(sensors,2) == 6
        electrode_model = 2;
    elseif ismember(zef.imaging_method, [1 4 5])
        electrode_model = 1;
    else
        electrode_model = 0;
    end
    ...
    if zef.attach_electrodes & electrode_model == 1
        sensors = zef_attach_sensors_volume(zef,sensors,'geometry',sensors_get_functions);
    elseif zef.attach_electrodes & electrode_model == 2
...
if not(isempty(sensors_get_functions{unique_sensors_aux_1(i)}))
    ...
    if ismember(zef.visualization_type,[3,4,5])

        if ismember(zef.visualization_type,[3,5])
            f_ind = frame_start;
        end
        ...
        for k = 1 : length(compartment_tags)
            ...
            if on_val
                i = i + 1;
                if visible_val
                    if ismember(i, aux_active_compartment_ind) &&  (ismember(zef.visualization_type, [3,4]))
                        ab_ind = find(aux_active_compartment_ind==i);
                        ...
                        if ismember(zef.visualization_type,[4])
                            ...
                            if zef.use_parcellation
                                ...
                                p_rec_aux =  ones(size(reuna_p{i},1),1).*zef.layer_transparency;
                                for p_ind = selected_list
                                    reconstruction(p_i_ind{p_ind}{2}{ab_ind}) = p_ind+1;
                                    p_rec_aux(unique(reuna_t{i}(p_i_ind{p_ind}{2}{ab_ind},:))) = zef.brain_transparency;
                                end
                            end
                            min_rec = 1;
                            max_rec = size(zef.parcellation_colormap,1);
                            ...
                            if zef.use_parcellation
                                ...
                                for p_ind = selected_list
                                    ...
                                    elseif zef.parcellation_type == 2
                                        reconstruction_aux(p_i_ind{p_ind}{2}{ab_ind}) = quantile(reconstruction(p_i_ind{p_ind}{2}{ab_ind}),zef.parcellation_quantile);
                                    elseif zef.parcellation_type == 3
                                        reconstruction_aux(p_i_ind{p_ind}{2}{ab_ind}) = quantile(sqrt(reconstruction(p_i_ind{p_ind}{2}{ab_ind})),zef.parcellation_quantile);
                                    elseif zef.parcellation_type == 4
                                        reconstruction_aux(p_i_ind{p_ind}{2}{ab_ind}) = quantile((reconstruction(p_i_ind{p_ind}{2}{ab_ind}).^(1/3)),zef.parcellation_quantile);
                                    elseif zef.parcellation_type == 5
                        ...
                        if ismember(i,aux_active_compartment_ind) && zef.use_inflated_surfaces && not(isempty(reuna_p_inf))
                        ...
                        if zef.brain_transparency < 1 || zef.use_parcellation
                            ...
                            if zef.use_parcellation
                                ...
                                set(h_surf_2{ab_ind},'FaceVertexAlpha',max(zef.brain_transparency,f_alpha_aux));
                            end
                            set(h_surf_2{ab_ind},'FaceAlpha','interp');
                            set(h_surf_2{ab_ind},'AlphaDataMapping','none');
                        end

                        if ismember(i,aux_active_compartment_ind) && cb_done == 0 && ismember(zef.visualization_type,[3])
                            cb_done = 1;
                        ...
                        if ismember(zef.visualization_type,[5]) && i == length(reuna_p)
                            ...
                            if ismember(i,aux_active_compartment_ind) && zef.use_inflated_surfaces && not(isempty(reuna_p_inf))
                                ...
                                [h_contour{i},h_contour_text{i}] = zef_plot_contour(zef,zef.contour_set,reconstruction,reuna_t{i},reuna_p{i});
                            else
                            ...
                            if zef.brain_transparency < 1 || zef.use_parcellation
                                ...
                                if zef.use_parcellation
                                    ...
                                    set(h_surf_2{i},'FaceVertexAlpha',max(zef.brain_transparency,f_alpha_aux));
                                end
                            ...
                            %set(h_surf,'ambientstrength',0.85);
                            set(h_surf,'facealpha',zef.layer_transparency);
                            %if not(zef.visualization_type==3);
                            lighting phong;
                            %end
                        end
        ...
        if iscell(volumetric_distribution) &&  ismember(zef.visualization_type,[3])
            view(zef.azimuth,zef.elevation);
        else
            view(get(zef.h_axes1,'view'));
        end
        if iscell(zef.top_reconstruction) &&  ismember(zef.visualization_type,[5])
            view(zef.azimuth,zef.elevation);
        else
            view(get(zef.h_axes1,'view'));
        end
        ...
        elseif iscell(zef.top_reconstruction) &&  ismember(zef.visualization_type,[5])
        ...
        if ismember(zef.visualization_type,[3,5])
            ...
            for f_ind = frame_start + frame_step : frame_step : frame_stop
                ...
                elseif ismember(zef.visualization_type,[5])
                    ...
                    if ismember(i,aux_active_compartment_ind) && zef.use_inflated_surfaces && not(isempty(reuna_p_inf))
                        ...
                        %[h_contour{i},h_contour_text{i}] = zef_plot_contour(zef,zef.contour_set,reconstruction,reuna_t{i},reuna_p_inf{i});
                    else
                        h_surf_2{i} = trisurf(reuna_t{i},reuna_p{i}(:,1),reuna_p{i}(:,2),reuna_p{i}(:,3),reconstruction,'edgecolor','none');
                        set(h_surf_2{i},'Tag','reconstruction');
                        %[h_contour{i},h_contour_text{i}] = zef_plot_contour(zef,zef.contour_set,reconstruction,reuna_t{i},reuna_p{i});
                ...
                zef_set_sliders_print(1,h_axes_image);
                camorbit(h_axes_image,frame_step*zef.orbit_1/movie_fps,frame_step*zef.orbit_2/movie_fps);
                lighting phong;
                ...
                if ismember(zef.visualization_type,[3])
                    h_text = text(0, 0.5, ['Time: ' num2str(zef.inv_time_1 + zef.inv_time_2/2 + frame_step*(f_ind - 1)*zef.inv_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
                elseif ismember(zef.visualization_type,[5])
                    h_text = text(0, 0.5, ['Time: ' num2str(zef.top_time_1 + zef.top_time_2/2 + frame_step*(f_ind - 1)*zef.top_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
                end
                set(h_text,'visible','on','fontsize',1500);
        ...
        sensor_patches = findobj(zef.h_axes1,'Type','Patch','Tag','sensor');
        uistack(sensor_patches,'top');
        try
            zef_plot_dpq('static');
        catch
    ...
    close(h_fig_aux);
    if  iscell(volumetric_distribution) &  zef.visualization_type == 3
        close(h_waitbar);
    end
    ...
    if  iscell(zef.top_reconstruction) &&  zef.visualization_type == 5
        close(h_waitbar);
    end
    %**************************************************************************
