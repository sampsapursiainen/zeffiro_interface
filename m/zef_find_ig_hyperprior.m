...
a = 1e-1*ones(size(relative_noise_std));
b = 50*ones(size(relative_noise_std));
...
p_val_vec = zef_gamma_gpu(x,shape_param_vec,1./( relative_noise_std(:, ones(size(shape_param_vec,2),1)).^2 .* (shape_param_vec - 1)));
