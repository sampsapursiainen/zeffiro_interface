...
cdata_info.frame_start = zef.frame_start;
cdata_info.frame_stop = zef.frame_stop;
cdata_info.frame_step = zef.frame_step;
...
elseif isequal(zef.volumetric_distribution_mode,3)
    volumetric_distribution = y_ES;
    if iscell(volumetric_distribution)
        for i = 1 : length(volumetric_distribution)
            volumetric_distribution{i} = zef.L*volumetric_distribution{i};
        end
    else
        volumetric_distribution = zef.L*volumetric_distribution;
    end
end

h_colorbar = findobj(zef.h_zeffiro,'-regexp','tag','Colorbar');
...
if isequal(zef.h_toggle_controls.UserData,1) || isempty(zef.h_toggle_controls.UserData)
...
if ismember(zef.visualization_type, [3,4])
    ...
    elseif ismember(zef.volumetric_distribution_mode, [2])
...
h_axes_text = findobj(zef.h_zeffiro,'tag','image_details');
...
loop_count = 0;
while loop_movie && loop_count <= zef.loop_movie_count
        ...
        if electrode_model == 1 || not(ismember(zef.imaging_method,[1,4,5]))
            for i = 1 : size(sensors,1)
                ...
                if eval(['zef.' zef.current_sensors '_names_visible'])
                    ...
                    set(h_text,'FontSize',zef.h_axes1.FontSize);
                end
 ...
 electrode_surface_index = 1;
    ...
    if ismember(zef.visualization_type,[3,4,5])
            ...
            if on_val
                ...
                if visible_val
                    if ismember(i, aux_active_compartment_ind) &&  (ismember(zef.visualization_type, [3,4]))
                        ...
                        set(zef.h_zeffiro,'colormap', eval([colormap_cell{zef.inv_colormap} '(' num2str(colortune_param) ',' num2str(colormap_size) ')']));
                        ...
                        elseif ismember(zef.visualization_type,[3])
                        ...
                        if ismember(zef.visualization_type,[3,4])
                            ...
                            if ismember(zef.volumetric_distribution_mode,[1, 3])
                                zef_plot_cone_field(zef,zef.h_axes1,f_ind);
                            end
                            ...
                            if ismember(i,aux_active_compartment_ind) && cb_done == 0 && ismember(zef.visualization_type,[3])
                                ...
                                set(h_text,'String',['Time: ' num2str(zef.inv_time_1 + zef.inv_time_2/2 + frame_step*(f_ind - 1)*zef.inv_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
        ...
        camva(zef.h_axes1,zef.cam_va);

        if zef.axes_visible
            set(zef.h_axes1,'visible','on');
            set(zef.h_axes1,'xGrid','on');
            set(zef.h_axes1,'yGrid','on');
            set(zef.h_axes1,'zGrid','on');
        else
            set(zef.h_axes1,'visible','off');
            set(zef.h_axes1,'xGrid','off');
            set(zef.h_axes1,'yGrid','off');
            set(zef.h_axes1,'zGrid','off');
        end
        %drawnow;
        ...
        if ismember(zef.visualization_type,[3,5])
            ...
            for f_ind = frame_start + frame_step : frame_step : frame_stop
                ...
                camorbit(zef.h_axes1,frame_step*zef.orbit_1/movie_fps,frame_step*zef.orbit_2/movie_fps);
                ...
                if ismember(zef.visualization_type,[3])
                    set(h_text,'string', ['Time: ' num2str(zef.inv_time_1 + zef.inv_time_2/2 + frame_step*(f_ind - 1)*zef.inv_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
                elseif ismember(zef.visualization_type,[5])
                    set(h_text,'string', ['Time: ' num2str(zef.top_time_1 + zef.top_time_2/2 + frame_step*(f_ind - 1)*zef.top_time_3,'%0.6f') ' s, Frame: ' num2str(f_ind) ' / ' num2str(length_reconstruction_cell) '.']);
                end
