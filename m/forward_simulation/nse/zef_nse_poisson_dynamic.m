...
Q_1_v = spdiags(1./c_vec,0,size(v_1_nodes,1),size(v_1_nodes,1))*Q_1;
Q_2_v = spdiags(1./c_vec,0,size(v_1_nodes,1),size(v_1_nodes,1))*Q_2;
Q_3_v = spdiags(1./c_vec,0,size(v_1_nodes,1),size(v_1_nodes,1))*Q_3;

Q_1_v = Q_1_v(i_node_ind,i_node_ind);
Q_2_v = Q_2_v(i_node_ind,i_node_ind);
Q_3_v = Q_3_v(i_node_ind,i_node_ind);
...
S_u = sparse(3*n_i_nodes,3*n_i_nodes,0);
S_u([i_nodes_aux],[i_nodes_aux]) = S(i_node_ind,i_node_ind); 
S_u(n_i_nodes + [i_nodes_aux],n_i_nodes + [i_nodes_aux]) = S(i_node_ind,i_node_ind); 
S_u(2*n_i_nodes + [i_nodes_aux],2*n_i_nodes + [i_nodes_aux]) = S(i_node_ind,i_node_ind); 
...
L([i_nodes_aux], [i_nodes_aux]) = L_11 + L_22 + L_33; %2*L_11 + L_22 + L_33;
L(n_i_nodes+[i_nodes_aux], n_i_nodes+[i_nodes_aux]) = L_11 + L_22 + L_33; %L_11 + 2*L_22 + L_33;
L(2*n_i_nodes+[i_nodes_aux], 2*n_i_nodes+[i_nodes_aux]) = L_11 + L_22 + L_33; %L_11 + L_22 + 2*L_33;
...
zef_waitbar(i/length(time_vec),h_waitbar,['NSE solver: compute, velocity norm: ' sprintf('%0.3g',norm(u))]);
...
for smooth_ind = 1 : nse_field.velocity_n_smoothing
...
%friction_vec = mu_aux.*(Q_1(i_node_ind,i_node_ind)'*u_aux_1./c_vec(i_node_ind)) + (u_aux_1./c_vec(i_node_ind)).*(Q_1(i_node_ind,i_node_ind)'*mu_vec(i_node_ind)) + mu_aux.*(Q_2(i_node_ind,i_node_ind)'*(u_aux_2./c_vec(i_node_ind))) + (u_aux_2./c_vec(i_node_ind)).*(Q_2(i_node_ind,i_node_ind)'*mu_vec(i_node_ind)) + mu_aux.*(Q_3(i_node_ind,i_node_ind)'*(u_aux_3./c_vec(i_node_ind))) + (u_aux_3./c_vec(i_node_ind)).*(Q_3(i_node_ind,i_node_ind)'*mu_aux);

friction_vec =  u_aux_1.*(Q_1_v*mu_aux)  + u_aux_2.*(Q_2_v*mu_aux) + u_aux_3.*(Q_3_v*mu_aux);

b(i_node_ind) = b(i_node_ind) + friction_vec;

if nse_field.nse_type == 2
        
q_1_u_1 = Q_1_v*u([i_nodes_aux]); 
q_1_u_2 = Q_1_v*u(n_i_nodes + [i_nodes_aux]);
q_1_u_3 = Q_1_v*u(2*n_i_nodes + [i_nodes_aux]);

q_2_u_1 = Q_2_v*u([i_nodes_aux]); 
q_2_u_2 = Q_2_v*u(n_i_nodes + [i_nodes_aux]);
q_2_u_3 = Q_2_v*u(2*n_i_nodes + [i_nodes_aux]);

q_3_u_1 = Q_3_v*u([i_nodes_aux]);
q_3_u_2 = Q_3_v*u(n_i_nodes + [i_nodes_aux]);
q_3_u_3 = Q_3_v*u(2*n_i_nodes + [i_nodes_aux]);

q_1_v_1 = Q_1_v*v([i_nodes_aux]); 
q_1_v_2 = Q_1_v*v(n_i_nodes + [i_nodes_aux]);
q_1_v_3 = Q_1_v*v(2*n_i_nodes + [i_nodes_aux]);

q_2_v_1 = Q_2_v*v([i_nodes_aux]);
q_2_v_2 = Q_2_v*v(n_i_nodes + [i_nodes_aux]);
q_2_v_3 = Q_2_v*v(2*n_i_nodes + [i_nodes_aux]);

q_3_v_1 = Q_3_v*v([i_nodes_aux]);
q_3_v_2 = Q_3_v*v(n_i_nodes + [i_nodes_aux]);
q_3_v_3 = Q_3_v*v(2*n_i_nodes + [i_nodes_aux]);
...
b(i_node_ind) = b(i_node_ind) +  nse_field.rho.*tr_vec.*c_vec(i_node_ind);
...
u_gu_vec_1 = q_1_u_1.*v([i_nodes_aux]) + q_2_u_1.*v(n_i_nodes + [i_nodes_aux]) + q_3_u_1.*v(2*n_i_nodes + [i_nodes_aux]);
u_gu_vec_2 = q_1_u_2.*v([i_nodes_aux]) + q_2_u_2.*v(n_i_nodes + [i_nodes_aux]) + q_3_u_2.*v(2*n_i_nodes + [i_nodes_aux]);
u_gu_vec_3 = q_1_u_3.*v([i_nodes_aux]) + q_2_u_3.*v(n_i_nodes + [i_nodes_aux]) + q_3_u_3.*v(2*n_i_nodes + [i_nodes_aux]);
    
n_p([i_nodes_aux]) = n_p([i_nodes_aux]) - nse_field.rho.*u_gu_vec_1.*c_vec(i_node_ind);
n_p(n_i_nodes + [i_nodes_aux]) = n_p(n_i_nodes + [i_nodes_aux]) - nse_field.rho.*u_gu_vec_2.*c_vec(i_node_ind);
n_p(2*n_i_nodes + [i_nodes_aux]) = n_p(2*n_i_nodes + [i_nodes_aux]) - nse_field.rho.*u_gu_vec_3.*c_vec(i_node_ind);
...
trace_strain_rate = abs(Q_1(:,i_node_ind)*u([i_nodes_aux]))./c_vec + abs(Q_2(:,i_node_ind)*u(n_i_nodes+[i_nodes_aux]))./c_vec + abs(Q_3(:,i_node_ind)*u(2*n_i_nodes+[i_nodes_aux]))./c_vec; 
...
nse_field.bv_vessels_1{i_aux}(i_node_ind) = gather(u([i_nodes_aux]));
nse_field.bv_vessels_2{i_aux}(i_node_ind) = gather(u(n_i_nodes+[i_nodes_aux]));
nse_field.bv_vessels_3{i_aux}(i_node_ind) = gather(u(2*n_i_nodes+[i_nodes_aux]));
