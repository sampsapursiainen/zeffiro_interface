...
zef.h_source_model.Items = arrayfun(@to_string, setdiff(zeffiro.SourceModel.variants, zeffiro.SourceModel.Error));
zef.h_source_model.ItemsData = arrayfun(@zeffiro.SourceModel.from, setdiff(zeffiro.SourceModel.variants, zeffiro.SourceModel.Error));
zef.source_model = zeffiro.SourceModel.from(zef.source_model);
if eq(zef.source_model, zeffiro.SourceModel.Error)
    warning("Invalid source model. Setting it as H(div)");
    zef.source_model = zeffiro.SourceModel.Hdiv;
end
zef.h_source_model.Value = zeffiro.SourceModel.from(zef.source_model);
