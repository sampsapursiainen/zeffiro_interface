...
noise_pros = evalin('base','zef.inv_synth_source(1,10)');
...
s_o = 1e-3*repmat(s_a,1,3).*s_o;
L = evalin('base','zef.L');
meas_data = zeros(size(L(:,1),1),1);
for i = 1 : size(s_p,1)
[s_min,s_ind] = min(sqrt(sum((source_positions - repmat(s_p(i,:),size(source_positions,1),1)).^2,2)));
meas_data = meas_data + s_o(i,1)*L(:,3*(s_ind-1)+1) + s_o(i,2)*L(:,3*(s_ind-1)+2) + s_o(i,3)*L(:,3*(s_ind-1)+3);
end
n_val = max(abs(meas_data));
meas_data = meas_data + (noise_pros/100)*abs(meas_data).*randn(size(meas_data));
