...
if  iscell(evalin('base','zef.reconstruction')) 
h_waitbar = waitbar(1,['Frame ' int2str(1) ' of ' int2str(number_of_frames) '.']);    
...
set(evalin('base','zef.h_zeffiro'),'colormap',gray(colormap_size));
end    
...
h_surf_2 = trisurf(reuna_t{i},reuna_p{i}(:,1),reuna_p{i}(:,2),reuna_p{i}(:,3),'edgecolor','none');
...
h_text = text(0, 0.5, ['Time: ' num2str(evalin('base','zef.inv_time_1') + evalin('base','zef.inv_time_2')/2 + (f_ind_aux - 1)*evalin('base','zef.inv_time_3'),'%0.9f') ' s']);
set(h_text,'visible','on');
set(h_axes_text,'layer','bottom');

else
    
h_surf = trimesh(reuna_t{i},reuna_p{i}(:,1),reuna_p{i}(:,2),reuna_p{i}(:,3),'edgecolor',color_str,'facecolor','none');
set(h_surf,'specularstrength',0.1);
set(h_surf,'diffusestrength',0.5);
set(h_surf,'ambientstrength',0.85);
set(h_surf,'edgealpha',evalin('base','zef.layer_transparency'));
