...
%   ];
%
%   A set of compartment names for which a segmentation will be generated.
%   The number of compartments should match that of generated by recon_all.
%
% - kwargs.motion_correction_reference_indices = 0
%
%   Each compartment named by kwargs.compartment_names will use a compartment
%   with a corresponding index in this list as a reference compartment during
%   motion correction via mri_mc.
%
%
% Outputs:
%
...
%
    ...
    arguments
        ...
        kwargs.motion_correction_reference_indices (:,1) double { mustBeInteger, mustBeNonnegative } = 0

        kwargs.compartment_names (:,1) string = [
            "lh_Lateral-Ventricle"; "lh_Inf-Lat-Vent"; "lh_CerebellumWM";
            "lh_CerebellumCortex"; "lh.thalamus"; "lh.caudate"; "lh.putamen";
            "lh.pallidum"; "3rd-Ventricle"; "4th-Ventricle"; "Brainstem";
            "lh.Hippocampus"; "lh.Amygdala"; "lh.Accumbens"; "LVentral_DC";
    ...
    motionCorrectionRefIndN = numel(kwargs.motion_correction_reference_indices) ;

    motionCorrectionArraysAligned = motionCorrectionRefIndN == numel(kwargs.compartment_names) || motionCorrectionRefIndN == 1 ;

    assert(motionCorrectionArraysAligned, "The number of motion correction reference indices did not match the number of given compartment names. If you wish to use the same reference for all compartments, give a single integer as a reference index.") ;
    ...
    for ii = 1 : numel(kwargs.motion_correction_reference_indices)

        if motionCorrectionRefIndN == 1

            cnum = kwargs.motion_correction_reference_indices(end);

        else

            cnum = kwargs.motion_correction_reference_indices(ii);

        end

        cname = kwargs.compartment_names(ii);
