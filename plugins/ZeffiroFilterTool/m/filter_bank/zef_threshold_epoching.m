function [processed_data] = zef_threshold_epoching(f, threshold_value, start_time, end_time, sampling_frequency)
...
while size(f_db, 2) >= size(epoch_data,2) && max(f_db) >= threshold_value 

    n_epochs = n_epochs + 1;
    epoch_point_ind = find(f >= threshold_value,1);
