...
zef.GMM.apps.main.ImportMenu.MenuSelectedFcn = 'if not(isempty(zef.save_file_path)) && prod(not(zef.save_file_path==0)); [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixature Model'',zef.save_file_path); else; [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixature Model''); end; if ~isequal(zef_aux_file,0) && ~isequal(zef_aux_path,0); zef_aux = load(fullfile(zef_aux_path,zef_aux_file)); if isfield(zef_aux,''zef_GMModel''); zef.GMM.model = zef_aux.zef_GMModel; zef.GMM.dipoles = zef_aux.zef_GMModelDipoles; elseif isfield(zef_aux,''zef_GMM''); zef_aux = zef_aux.zef_GMM; if isfield(zef_aux,''model''); zef.GMM.model = zef_aux.model; end; if isfield(zef_aux,''dipoles''); zef.GMM.dipoles = zef_aux.dipoles; end; if isfield(zef_aux,''parameters''); zef.GMM.parameters = zef_aux.parameters; zef_GMM_update; end; if isfield(zef_aux,''reconstruction''); zef.reconstruction = zef_aux.reconstruction; end; if isfield(zef_aux,''time_variables''); zef.GMM.time_variables = zef_aux.time_variables; end; if isfield(zef_aux,''amplitudes''); zef.GMM.amplitudes = zef_aux.amplitudes; end; end; end; clear zef_aux zef_aux_file zef_aux_path;';
