...
zef.GMM.apps.main.ImportButton.ButtonPushedFcn = 'if not(isempty(zef.save_file_path)) && prod(not(zef.save_file_path==0)); [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixature Model'',zef.save_file_path); else; [zef_aux_file,zef_aux_path] = uigetfile(''*.mat'',''Select Gaussian Mixature Model''); end; if ~isequal(zef_aux_file,0) && ~isequal(zef_aux_path,0); zef_aux = load(fullfile(zef_aux_path,zef_aux_file)).zef_GMM; if isfield(zef_aux,''model''); zef.GMM.model = zef_aux.model; end; if isfield(zef_aux,''dipoles''); zef.GMM.dipoles = zef_aux.dipoles; end; if isfield(zef_aux,''parameters''); zef.GMM.parameters = zef_aux.parameters; zef_GMM_update; end; end; clear zef_aux zef_aux_file zef_aux_path;';
