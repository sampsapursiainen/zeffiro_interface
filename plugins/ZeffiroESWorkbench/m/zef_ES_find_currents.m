...
for i = 1 : lattice_size
    ...
    while  j <  lattice_size
        ...
        for parallel_ind = 1 : p_ind_max
            j = j + 1;
            zef_data_2.y_ES_interval.run_time{i,j}                                  = run_time{parallel_ind};
            ...
            zef_data_2.y_ES_interval.nnz{i,j}                          = zef_ES_rwnnz(y_ES{parallel_ind},zef_data.relative_weight_nnz,zef_data.score_dose);
            ...
            if ismember(flag{parallel_ind},[1,3]) && norm(y_ES{parallel_ind},2) > 0
                for running_index = 1:length(source_position_index{parallel_ind})
                    ...
                    zef_data_2.y_ES_interval.field_source(running_index).amplitude{i,j}            = mean(sum(volumetric_current_density{parallel_ind}(:,source_running_ind).*repmat(vec_1',1,length(source_running_ind))./norm_vec_1));
                    ...
                    zef_data_2.y_ES_interval.field_source(running_index).relative_error{i,j}       = norm(vec_1'/norm(vec_1)-vec_2/norm(vec_2));
                ...
                for running_index = 1:length(source_position_index{parallel_ind})
                    ...
                    zef_data_2.y_ES_interval.field_source(running_index).amplitude{i,j}            = 0;
