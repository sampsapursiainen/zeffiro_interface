...
if evalin('base','zef.ES_search_method') == 1
    ...
    L_ES_projection   = [singular_value_max*TolFun*L_aux(J_x_ES,:)  ; L_ES_projection];
end
        ...
        if alpha <= 0
            ...
            if evalin('base','zef.h_ES_CVXCheckBox.Value')
               % [y_ES,~,flag_val] = zef_cvx_linprog(g, ...
                ...
                %    [-x_ES_projection ; x_ES_projection; alpha*evalin('base','zef.ES_total_max_current')], ...
                %    [], ...
                %    [], ...
                ...
                %    [ evalin('base','zef.ES_max_current_channel')*ones(size(L_ES_projection,2),1); max(source_magnitude)*ones(size(L_ES_projection,1)-size(L_ES_projection,2),1); alpha*evalin('base','zef.ES_max_current_channel')*ones(size(L_ES_projection,2),1) ],...
                %    opts);
                 [y_ES,~,flag_val] = zef_cvx_linprog(g, ...
                    [-L_ES_projection -eye(size(L_ES_projection,1))  ; L_ES_projection -eye(size(L_ES_projection,1)); zeros(1, size(L_ES_projection,1)) ones(1, size(L_ES_projection,2)) ], ...
                    [-x_ES_projection ; x_ES_projection; evalin('base','zef.ES_total_max_current')], ...
                    [], ...
                    [], ...
                    [-evalin('base','zef.ES_max_current_channel')*ones(size(L_ES_projection,2),1); TolFun*max(source_magnitude)*ones(size(L_ES_projection,1)-size(L_ES_projection,2)-length(source_position_index),1); zeros(size(L_ES_projection,2)+length(source_position_index),1) ], ...
