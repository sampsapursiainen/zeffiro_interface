...
for i_aux = 1 : size(vec, 2)-1 % Last one are the ES channels.
    ...
    if not(isempty(vec{:, i_aux}))
        ...
        if i_aux == 13 % alpha
            zef.h_ES_optimizer_properties_table.Data{i_aux, 2} = round(vec{:, i_aux}(sc),4);
        elseif i_aux == 14 % beta or k-val
            zef.h_ES_optimizer_properties_table.Data{i_aux, 2} = round(vec{:, i_aux}(sr),4);
        else
            zef.h_ES_optimizer_properties_table.Data{i_aux, 2} = vec{:, i_aux}(sr,sc);
        end

        if i_aux <= 12
            zef.h_ES_optimizer_properties_table.Data{i_aux, 3} = zef_lattice_deviation(vec{:, i_aux},'avg',0.5,sr,sc);
            zef.h_ES_optimizer_properties_table.Data{i_aux, 4} = zef_lattice_deviation(vec{:, i_aux},'max',0.5,sr,sc);
        end
...
zef.h_ES_optimizer_properties_table.Data{i_aux,2} = sum(vec{:,12},'all');
