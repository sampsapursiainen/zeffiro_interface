...
zef_data.search_method          = zef.ES_search_method;
...
zef_data.solver_package         = zef.ES_solver_package;
            ...
            if ismember(flag{parallel_ind}, [1, 3]) && norm(y_ES{parallel_ind}, 2) > 0
                for running_index = 1:length(source_position_index{parallel_ind})
                    ...
                    if isequal(zef.ES_roi_range, 0)
                        ...
                        source_running_ind = rangesearch(zef_data.source_positions, zef_data.source_positions(source_position_index{parallel_ind}(running_index),:), zef.ES_roi_range);
