...
point_ind = find(strip_struct.points(:,3)>=1.5+2.0*(contact_index-1) & strip_struct.points(:,3)<=3.0+2*(contact_index-1));
end

if isequal(strip_struct.strip_model,2)
    if contact_index == 1
        contact_index_aux = 1;
point_ind = find(strip_struct.points(:,3)>=1.5+2.0*(contact_index_aux-1) & strip_struct.points(:,3)<=3.0+2*(contact_index_aux-1));
...
point_ind = find(strip_struct.points(:,3)>=1.5+2.0*(contact_index_aux-1) & strip_struct.points(:,3)<=3.0+2*(contact_index_aux-1));
angle_aux = cart2pol(strip_struct.points(point_ind,1),strip_struct.points(point_ind,2));
...
point_ind = find(strip_struct.points(:,3)>=0.75+0.75*(row_ind_aux-1) & strip_struct.points(:,3)<=1.5+0.75*(row_ind_aux-1));
...
point_ind_aux = find(sqrt(sum((strip_struct.points(point_ind,:)'-center_point_aux).^2))<=0.5*0.75)
point_ind = point_ind(point_ind_aux);
end

tri_ind = find(sum(ismember(strip_struct.triangles,point_ind),2)==3);
triangles = strip_struct.triangles(tri_ind,:);
