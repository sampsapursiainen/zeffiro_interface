...
function [exp_em_multires_dec, exp_em_multires_ind, exp_em_multires_count] = exp_em_make_multires_dec

n_decompositions = evalin('base','zef.exp_em_multires_n_decompositions');
[n_levels] = evalin('base','zef.exp_em_multires_n_levels');
...
exp_em_multires_dec{n_rep}{n_levels} = [1:size_center_points]';
exp_em_multires_ind{n_rep}{n_levels} = [1:size_center_points]';
exp_em_multires_count{n_rep}{n_levels} = ones(size_center_points,1);

par_num = evalin('base','zef.parallel_vectors');
exp_em_multires_sparsity = evalin('base','zef.exp_em_multires_sparsity');
...
exp_em_multires_dec{n_rep}{k} = aux_ind(1:size_source_points)';
...
exp_em_multires_ind{n_rep}{k} = source_interpolation_aux;
[aux_vec, i_a, i_c] = unique(source_interpolation_aux);
exp_em_multires_count{n_rep}{k} = accumarray(i_c,1);
