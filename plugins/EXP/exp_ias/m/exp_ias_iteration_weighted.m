...
function [z] = exp_ias_iteration_weighted(void)
...
n_ias_map_iter = evalin('base','zef.exp_ias_n_map_iterations');
n_L1_iter = evalin('base', 'zef.exp_ias_n_L1_iterations');
beta = evalin('base','zef.exp_ias_beta');
theta0 = evalin('base','zef.exp_ias_theta0');
std_lhood = evalin('base','zef.exp_ias_likelihood_std');
sampling_freq = evalin('base','zef.exp_ias_sampling_frequency');
high_pass = evalin('base','zef.exp_ias_low_cut_frequency');
low_pass = evalin('base','zef.exp_ias_high_cut_frequency');
number_of_frames = evalin('base','zef.exp_ias_number_of_frames');
...
%__Get measurements__
if iscell(evalin('base','zef.measurements'));
f = evalin('base',['zef.measurements{' int2str(evalin('base','zef.exp_ias_data_segment')) '}']);
else
f = evalin('base','zef.measurements');
end
%_Normalize measurements_
data_norm = 1;
if evalin('base','zef.exp_ias_normalize_data')==1;
data_norm = max(abs(f(:))); 
%std_lhood = std_lhood^2;
elseif evalin('base','zef.exp_ias_normalize_data')==2;
data_norm = max(sqrt(sum(abs(f).^2)));
%std_lhood = std_lhood^2;
elseif evalin('base','zef.exp_ias_normalize_data')==3;
...
if size(f_data,2) > 1  
if evalin('base','zef.exp_ias_time_2') >=0 && evalin('base','zef.exp_ias_time_1') >= 0 && 1 + sampling_freq*evalin('base','zef.exp_ias_time_1') <= size(f_data,2);
f = f_data(:, max(1, 1 + floor(sampling_freq*evalin('base','zef.exp_ias_time_1')+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_ias_time_3'))) : min(size(f_data,2), 1 + floor(sampling_freq*(evalin('base','zef.exp_ias_time_1') + evalin('base','zef.exp_ias_time_2'))+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_ias_time_3'))));
end

%__Initialize x (= z_vec) and theta_i's__
...
[m,n] = size(L);
z_vec = zeros(n,1);
gamma =zeros(length(z_vec),1)+W.*(beta+1/q-1)./theta0;
...
%__Update theta__
    if q==2
        ...
        w = 1./sqrt(gamma);
        %W = diag(w.^2);
        %x = W*At*inv(A*W*At+ eye(m))*b; 
       
        %inversion matrix lemma is used here!
        s_1 = w.^2 .* L_aux'*f/std_lhood;
        s_2 = L_aux * s_1;
        D_sqrt_A_tr = bsxfun(@times,L_aux',w);
        s_2 = linsolve(gather(eye(m,m) + D_sqrt_A_tr' * D_sqrt_A_tr),gather(s_2),opt);
        z_vec = s_1 - w.^2 .* (L_aux' * s_2); 
        %_ gamma update _
        %gamma = W.*(beta+1/q-1)./(theta0+0.5*abs(z_vec).^q);
        gamma = 0.5*std_lhood*max(abs(f))*sum(L_aux.^2,1)'./(theta0+abs(z_vec));
        %gamma = 0.8*std_lhood*max(abs(f))*norm(L*z_vec)./(theta0+z_vec.^2);
        end
    else
        %gamma = 0.5*max((At*b));
        %betas = (alpha+1)/gamma;
...
if ismember(source_direction_mode,[1 2])
z_aux(s_ind_1) = gather(z_vec);
end
if ismember(source_direction_mode,[3])
z_aux(s_ind_2) = gather(z_vec);
end
