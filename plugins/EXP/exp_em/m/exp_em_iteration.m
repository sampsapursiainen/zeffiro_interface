...
function [z] = exp_em_iteration(void)
...
snr_val = evalin('base','zef.exp_em_snr');
pm_val = evalin('base','zef.inv_prior_over_measurement_db');
amplitude_db = 20; 
pm_val = pm_val - amplitude_db;
std_lhood = 10^(-snr_val/20);
sampling_freq = evalin('base','zef.exp_em_sampling_frequency');
high_pass = evalin('base','zef.exp_em_low_cut_frequency');
low_pass = evalin('base','zef.exp_em_high_cut_frequency');
number_of_frames = evalin('base','zef.exp_em_number_of_frames');
...
if ismember(hyper_type,[1,2])
    if evalin('base','zef.exp_em_normalize_data')==1;
...
f = evalin('base',['zef.measurements{' int2str(evalin('base','zef.exp_em_data_segment')) '}']);
else
...
elseif evalin('base','zef.exp_em_normalize_data')==2;
data_norm = max(sqrt(sum(abs(f).^2)));
%std_lhood = std_lhood^2;
elseif evalin('base','zef.exp_em_normalize_data')==3;
...
if size_f > 1  
if evalin('base','zef.exp_em_time_2') >=0 0 && evalin('base','zef.exp_em_time_1') >= 0 & 1 + sampling_freq*evalin('base','zef.exp_em_time_1') <= size_f;
f = f_data(:, max(1, 1 + floor(sampling_freq*evalin('base','zef.exp_em_time_1')+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_em_time_3'))) : min(size_f, 1 + floor(sampling_freq*(evalin('base','zef.exp_em_time_1') + evalin('base','zef.exp_em_time_2'))+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_em_time_3'))));
end
end
...
n_ias_map_iter = evalin('base','zef.exp_em_n_map_iterations');
n_L1_iter = evalin('base','zef.exp_em_n_L1_iterations');
