...
function [z] = exp_ias_iteration_multires(void)
...
n_multires = evalin('base','zef.exp_ias_multires_n_levels');
sparsity_factor = evalin('base','zef.exp_ias_multires_sparsity');
...
%theta0_0 = theta0;
snr_val = evalin('base','zef.exp_ias_multires_snr');
pm_val = evalin('base','zef.inv_prior_over_measurement_db');
amplitude_db = 20; 
pm_val = pm_val - amplitude_db;
std_lhood = 10^(-snr_val/20);
sampling_freq = evalin('base','zef.exp_ias_multires_sampling_frequency');
high_pass = evalin('base','zef.exp_ias_multires_low_cut_frequency');
low_pass = evalin('base','zef.exp_ias_multires_high_cut_frequency');
number_of_frames = evalin('base','zef.exp_ias_multires_number_of_frames');
...
f = evalin('base',['zef.measurements{' int2str(evalin('base','zef.exp_ias_multires_data_segment')) '}']);
else
...
data_norm = 1;
if evalin('base','zef.exp_ias_multires_normalize_data')==1
data_norm = max(abs(f(:))); 
%std_lhood = std_lhood^2;
elseif evalin('base','zef.exp_ias_multires_normalize_data')==2
data_norm = max(sqrt(sum(abs(f).^2)));
%std_lhood = std_lhood^2;
elseif evalin('base','zef.exp_ias_multires_normalize_data')==3
...
if size_f > 1  
if evalin('base','zef.exp_ias_multires_time_2') >=0 0 && evalin('base','zef.exp_ias_multires_time_1') >= 0 & 1 + sampling_freq*evalin('base','zef.exp_ias_multires_time_1') <= size_f;
f = f_data(:, max(1, 1 + floor(sampling_freq*evalin('base','zef.exp_ias_multires_time_1')+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_ias_multires_time_3'))) : min(size_f, 1 + floor(sampling_freq*(evalin('base','zef.exp_ias_multires_time_1') + evalin('base','zef.exp_ias_multires_time_2'))+sampling_freq*(f_ind - 1)*evalin('base','zef.exp_ias_multires_time_3'))));
end
end
if size_f > 1
f = mean(f,2);
...
n_L1_iter = evalin('base','zef.exp_ias_multires_n_L1_iterations');

if evalin('base','zef.use_gpu') == 1 & gpuDeviceCount > 0
f = gpuArray(f);
end 

exp_ias_multires_dec =  evalin('base','zef.exp_ias_multires_dec');
exp_ias_multires_ind =  evalin('base','zef.exp_ias_multires_ind');
multires_count = evalin('base','zef.exp_ias_multires_count');
n_iter = evalin('base','zef.exp_ias_multires_n_iter');
if length(n_iter) < n_multires
    n_iter = n_iter(1)*ones(1,n_multires);
end
%mr_sparsity = evalin('base','zef.exp_ias_multires_sparsity');
...
theta =zeros(size(L_aux,2),1)+(beta+1/q-1)./theta0;
end 
...
z_vec = z_vec_aux/(n_decompositions*weight_vec_aux);
