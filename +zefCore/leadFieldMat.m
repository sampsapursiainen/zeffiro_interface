...
%
    ...
    arguments
        ...
        sensors   (:,1)                        { mustBeA(sensors, "zefCore.Sensor") }
        modality  (1,1) zefCore.LeadFieldModality
        acI       (:,1) double                 { mustBeInteger, mustBePositive }
        params    (1,1) zefCore.LeadFieldParams
    end

    import zefCore.LeadFieldModality

    disp("Computing surface triangles of mesh…")

    surfTri = zefCore.tetraSurfaceTriangles ( tetra ) ;
    ...
    [ ~, e2nI ] = zefCore.attachSensors(sensors.positions',surfN,[]) ;

    s2nI = surfTri (e2nI) ;

    disp ("Computing surface triangle areas…")

    [ triA, ~ ] = zefCore.triangleAreas(nodes',surfTri);
    ...
    [ reA, imA ] = zefCore.stiffnessMat(nodes,tetra,tetV,sigma);

    % Compute uninterpolated lead field.

    if modality == LeadFieldModality.EEG

        L = zefCore.eegLeadField ( nodes, surfTri, sensors, triA, s2nI, reA, imA, pcgTol=params.solver_tolerance ) ;
    ...
    peeledI = zefCore.peelSourcePositions ( nodes, tetra, acI, params.acceptable_source_depth ) ;

    % Then build a dipole interpolation matrix.

    [interpM, ~] = zefCore.leadFieldInterpolation ( ...
        nodes, ...
        tetra, ...
        acI, ...
        params.source_model, ...
